...
public class QuestionController {
    ...
    public ResponseEntity<ApiResponse<List<QuestionDTO>>> listAll() {
        List<QuestionDTO> dtos = service.getAllQuestions().stream().map(QuestionDTO::new).collect(Collectors.toList());
    ...
    public ResponseEntity<ApiResponse<QuestionDTO>> getOne(@PathVariable Long id) {
        return service.getQuestionById(id).map(q -> ResponseEntity.ok(new ApiResponse<>(new QuestionDTO(q)))).orElse(ResponseEntity.notFound().build());
    }
